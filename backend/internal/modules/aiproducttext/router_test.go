...
func TestRegisterUnavailableDependenciesFailClosed(t *testing.T) {
	for _, svc := range []*Service{nil, {}} {
		...
		for _, tc := range []struct {
			...
			{http.MethodGet, "/products/ai-text/batches"},
			{http.MethodPost, "/products/ai-text/batches/check"},
		} {
			...
			if err := json.Unmarshal(rec.Body.Bytes(), &body); err != nil || body.Code != response.CodeInternalError || body.Message != "aiproducttext unavailable" {
