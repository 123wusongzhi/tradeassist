...
if (!/^p7v2-baseline-r3b-recovery6-[a-z0-9_-]+$/.test(runId)) {
  throw new Error('P7-V2-R3B-FAST-CLOSE-R3-FORMAL requires a unique Recovery6 baseline run ID');
}
...
try {
  ...
  const registry = readJSON(registryPath) || { phase: 'P7-V2-R3B-FAST-CLOSE-R3-FORMAL', baselines: [] };
  ...
  registry.phase = 'P7-V2-R3B-FAST-CLOSE-R3-FORMAL';
  ...
  if (freezeRevalidation.status !== 'passed') throw new Error(`runtime freeze immutable revalidation failed after baseline freeze: ${freezeRevalidation.rebuildError || 'immutable mismatch'}`);
  const lifecycle = validateFormalExecutionLifecycle({ previousState: 'baseline_completed', nextState: 'baseline_frozen' });
  writeJSON('docs/p7-v2-r3b-runtime-freeze-lifecycle-validation.json', lifecycle);
  if (lifecycle.status !== 'passed') throw new Error(`baseline lifecycle validation failed: ${lifecycle.issues.map((issue) => issue.issue).join('; ')}`);
  console.log(JSON.stringify({ phase: 'P7-V2-R3B-FAST-CLOSE-R3-FORMAL', kind: 'baseline', runId, freeze: 'passed', sha256: frozen.sha256 }, null, 2));
} catch (error) {
