...
func collectFailureHint(code, source string, sameURLSucceeded, inBatch bool) string {
	code = strings.ToUpper(strings.TrimSpace(code))
	...
	if sameURLSucceeded {
		if !inBatch {
			return "该链接此前采集成功，本次更可能是临时访问、登录状态、限流或目标站点风控导致。建议确认所选采集引擎状态后稍后重试。"
		}
...
func (s *Service) enrichTaskDTO(ctx context.Context, t *CollectTask) TaskDTO {
	...
	if t.Status == StatusFailed || t.Status == StatusRetrying {
		...
		dto.FailureHint = collectFailureHint(code, t.Source, sameOK, inBatch)
	}
	return dto
}
