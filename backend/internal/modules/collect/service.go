...
type Service struct {
	...
	EngineRouter            *CollectorEngineRouter
	Redis                   *rdb.Client
	QueueName               string
	QueueEnabled            bool
	BatchMaxURLs            int
...
func (s *Service) RunCollectJob(parent context.Context, taskID uuid.UUID, workerID string) {
	...
	// The selected engine is immutable for the lifetime of the task. In
	// particular, an OpenCLI failure never falls back to Playwright silently.
	outcome, err := engineRouter.Collect(
		ctx,
		resolvedEngine,
		task.Source,
		task.SourceURL,
