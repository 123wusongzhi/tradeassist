...
export default function CollectBatchesPage() {
  ...
  const formEngineWatch = Form.useWatch('engine', form) as CollectEngine | undefined;
  ...
  useEffect(() => {
    if (isTaobaoTmallSource(formSourceWatch)) {
  ...
  }, [defaultEngine, engineResolved, form, formSourceWatch]);

  useEffect(() => {
    const sync = () => setTaskPolling(document.visibilityState === 'hidden' || !drawerOpen ? undefined : POLL_MS);
    sync();
    document.addEventListener('visibilitychange', sync);
    return () => document.removeEventListener('visibilitychange', sync);
  }, [drawerOpen]);
  ...
  const batchSourcePlaceholder = useMemo(() => {
    ...
    const line = (p?.urlPatterns?.length ? p.urlPatterns[0] : undefined) ?? 'https://detail.1688.com/offer/...';
    return `${line}\n${line}`;
  ...
  const batchColumns: ProColumns<CollectBatchRow>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        width: 112,
        valueType: 'select',
        valueEnum: Object.fromEntries(Object.entries(COLLECT_BATCH_STATUS).map(([k, v]) => [k, { text: v.text }])),
        render: (_, row) => {
  ...
  const taskColumns: ProColumns<CollectTaskRow>[] = [
    ...
    },
    {
      ...
      search: false,
      render: (_, row) => {
        if (!isTaobaoTmallSource(row.source)) return '—';
    ...
    },
    {
      ...
      render: (_, row) => {
        ...
        if (row.sourceUrl) {
          actions.unshift(
            <Button key="open" type="link" size="small" href={row.sourceUrl} target="_blank" rel="noreferrer">
              原链接
            </Button>,
          );
        }
  ...
  return (
    <TmPageContainer title="批量采集" subTitle="一次提交多条商品链接，批量采集并生成商品草稿。">
      <ProCard variant="outlined" style={{ marginBottom: 16 }} bodyStyle={{ paddingBottom: 8 }}>
        {sourceFromQuery && providers.length > 0 && !batchProviders.some((p) => p.source === sourceFromQuery) ? (
          <Alert
            type="info"
            showIcon
        ...
        <Form
          ...
          onFinish={async (vals) => {
            ...
            setSubmitting(true);
            try {
              const res = await createCollectBatch({
                source: src,
                urls,
                engine,
              });
              ...
              const msg = mapCollectErrorMessage(e, src, engine);
              if (msg.includes('登录')) {
                message.error(TAOBAO_TMALL_BATCH_LOGIN_BLOCK_MSG);
        ...
        >
          <Space direction="vertical" style={{ width: '100%' }} size="middle">
            ...
            {taobaoUrlPreview && (taobaoUrlPreview.invalid.length > 0 || taobaoUrlPreview.unsupported.length > 0) ? (
              <Alert
                type="warning"
                showIcon
            ...
            <Form.Item label="采集平台" name="source" rules={[{ required: true }]}>
              <Select style={{ maxWidth: 320 }} options={batchSelectOpts} placeholder="选择批量采集平台" />
            </Form.Item>
            {isTaobaoTmallSource(formSourceWatch) ? (
              <Form.Item label="采集引擎" name="engine">
                <Segmented<CollectEngine> disabled={!engineResolved} options={collectEngineOptions(engineStatus)} />
              </Form.Item>
            ) : null}
            {isTaobaoTmallSource(formSourceWatch) && formEngineWatch === 'opencli' ? (
              <Alert
            ...
            <Form.Item
              label={
                <span>
                  ...
                  {isTaobaoTmallSource(formSourceWatch) ? <Tag color="blue">每批最多 {batchMaxItems} 条</Tag> : null}
                </span>
              }
              name="urls"
          ...
          <>
                  ...
                  <Space wrap>
                    ...
                    <span>
                      ...
                      {Math.max(
                        0,
                        Math.round(
                          (new Date(activeBatch.finishedAt).getTime() - new Date(activeBatch.createdAt).getTime()) /
                            1000,
                        ),
                      )}
                      秒
            ...
            <ProCard variant="outlined" size="small" style={{ marginBottom: 16 }} bodyStyle={{ padding: '12px 16px' }}>
              <Space wrap size="middle">
                ...
                {(activeBatch.blockedCount ?? 0) > 0 ? <Tag color="orange">风控 {activeBatch.blockedCount}</Tag> : null}
