...
type Config struct {
	...
	// CollectorBaseURL is the Playwright collector HTTP base. It is kept as the
	...
	// host-side OpenCLI Bridge. The Playwright collector remains independent.
	OpenCLIBridgeEnabled bool
	// OpenCLIBridgeBaseURL is the minimal host-side OpenCLI Bridge HTTP base.
	OpenCLIBridgeBaseURL string
	// OpenCLIBridgeToken authenticates backend requests to a remotely bound bridge.
	OpenCLIBridgeToken string
	// OpenCLIBridgeTimeoutSeconds caps OpenCLI collect calls.
	OpenCLIBridgeTimeoutSeconds int
	// CollectDefaultEngineTaobaoTmall is used when callers omit engine.
	CollectDefaultEngineTaobaoTmall string
...
func Load() (*Config, error) {
	...
	cfg := &Config{
		...
		CollectorBaseURL: strings.TrimRight(strings.TrimSpace(firstNonEmpty(
		...
		OpenCLIBridgeToken:          strings.TrimSpace(os.Getenv("OPENCLI_BRIDGE_TOKEN")),
		OpenCLIBridgeTimeoutSeconds: atoiOrDefault(os.Getenv("OPENCLI_BRIDGE_TIMEOUT_SECONDS"), 120),
		CollectDefaultEngineTaobaoTmall: strings.ToLower(strings.TrimSpace(firstNonEmpty(
			os.Getenv("COLLECT_DEFAULT_ENGINE_TAOBAO_TMALL"),
			"opencli",
		))),
