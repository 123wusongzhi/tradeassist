...
export const PLATFORM_PUBLISH_FIELD_LABEL: Record<string, string> = {
  ...
  description_category_id: '类目 ID',
  type_id: '商品类型 ID',
  currency_code: '币种',
  vat: '增值税率',
  default_brand: '默认品牌',
  default_type: '默认类型',
  default_country: '默认原产国',
  default_manufacturer: '默认制造商',
  default_depth: '默认深度',
  default_attributes: '补充必填属性 JSON',
  auto_fill_attributes: '自动匹配类目属性',
  ai_auto_fill: 'AI 自动补全缺失必填属性',
...
export const PLATFORM_PUBLISH_FIELD_HELP: Record<string, string> = {
  ...
  description_category_id: 'Ozon 叶子类目 ID（description_category_id），上品必填',
  type_id: 'Ozon 商品类型 ID（type_id），上品必填',
  currency_code: 'Ozon 商品币种，跨境卖家按后台合同币种选择',
  vat: 'Ozon 增值税率取值',
  default_brand: '商品参数缺少品牌时使用；自动匹配 Ozon 字典值',
  default_type: '商品参数缺少类型（Тип）时使用；自动匹配 Ozon 字典值',
  default_country: '商品参数缺少原产国时使用，例如 China / Китай',
  default_manufacturer: '商品参数缺少制造商时使用',
  default_depth: 'Ozon 商品深度，单位 mm',
  default_attributes: '格式 {"属性ID":"值"}，例如 {"85":"Apple","4389":"China"}',
  auto_fill_attributes: '按商品参数与默认值自动匹配类目必填属性；关闭后只提交标题/描述/图片/价格',
  ai_auto_fill: '规则匹配后仍有缺失必填属性时，用已配置的 AI 能力生成建议值并自动匹配字典；AI 未配置或失败时降级跳过，不阻塞上品',
...
export const PLATFORM_PUBLISH_FIELD_PLACEHOLDER: Record<string, string> = {
  ...
  description_category_id: '叶子类目 ID，如 200001240',
  type_id: '商品类型 ID，如 93488',
  default_brand: '品牌名称',
  default_type: '类型名称',
  default_country: '原产国，如 China',
  default_manufacturer: '制造商名称',
  default_depth: '如 10',
  default_attributes: '{"85":"Apple"}',
  ai_auto_fill: '推荐开启',
...
const FIELD_SECTION: Record<string, PublishFieldSectionKey> = {
  ...
  type_id: 'category',
  default_brand: 'category',
  default_type: 'category',
  default_country: 'category',
  default_manufacturer: 'category',
  ...
  vat: 'product',
  default_attributes: 'advanced',
  auto_fill_attributes: 'publish',
  ai_auto_fill: 'publish',
