...
> 状态：2026-08-09。当前为源码预览版，支持在 Chrome / Edge 当前打开的
> 淘宝、天猫、1688 商品详情页中采集。正式面向普通用户分发前，还需要发布签名扩展包或上架
...
- 只用本扩展即可完成淘宝/天猫/1688 单商品采集，不需要启动 Playwright Collector、
...
浏览器侧边栏是淘宝/天猫/1688 日常单商品采集的推荐入口。用户继续使用自己已经登录的平台
...
1. 在已连接扩展的同一浏览器中打开淘宝、天猫或 1688 商品详情页。
...
- 页面读取权限只覆盖已支持的淘宝/天猫详情域名，以及 1688 的
  `detail.1688.com`、`m.1688.com`；不申请 `*.1688.com` 全站权限。远程 TradeMind
  地址在配对时按具体 origin 单独申请。
- 1688 当前页必须使用 HTTPS，且 URL 必须包含可解析的数字 offer ID；扩展与 Backend
  使用相同边界拒绝首页、空 offer 路径、非数字 ID 和其他 1688 子域。
- Backend 将兼容输入 `taobao` 规范化为 `taobao_tmall`，并以已验证任务的
  `source` / `sourceUrl` 作为草稿落库值；结果 payload 缺少来源或与任务来源不匹配时拒绝。
- 非 loopback 地址强制使用 HTTPS；可选网络权限不包含任意远程 HTTP。
- 平台适配器随扩展构建产物发布，不允许运行时远程加载 JavaScript。

## 当前限制

- 支持淘宝/天猫/1688 当前页单商品采集；不把 1688 扩展采集接入 Playwright/OpenCLI engine router。
- 1688 阶梯价完整保存在草稿 `rawData.raw.priceTiers`；`SKU.price` 为首档或 SKU 单价，不是唯一成交价。
- 不支持扩展侧批量、定时或无人值守采集。
...
确认当前标签页是 `https` 的淘宝/天猫商品详情页，或 `detail.1688.com`、`m.1688.com`
上带数字 offer ID 的 1688 商品详情页，而不是搜索、登录、验证码或店铺首页。平台新增
域名时必须通过代码发布增加精确权限，不能远程下发脚本绕过审核。
