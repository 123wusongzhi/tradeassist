...
func TestRecoverLeaseExpiredDoesNotClearRenewedLease(t *testing.T) {
	db, err := gorm.Open(sqlite.Open(fmt.Sprintf("file:inventory_recovery_%s?mode=memory&cache=shared", uuid.New())), &gorm.Config{})
	...
	require.NoError(t, db.Create(&InventorySyncTask{HardDeleteBase: model.HardDeleteBase{ID: id}, TenantID: 0, ProductID: uuid.New(), ShopID: uuid.New(), Platform: "douyin_shop", TaskType: "sync", Mode: "manual", Status: StatusRunning, LockedBy: &worker, LockedUntil: &past, LockVersion: 1}).Error)
	var renew sync.Once
	require.NoError(t, db.Callback().Update().Before("gorm:update").Register("inventory_renew_before_recovery", func(tx *gorm.DB) {
		renew.Do(func() {
			tx.Exec("UPDATE inventory_sync_tasks SET locked_until = ?, heartbeat_at = ? WHERE id = ?", future, future, id)
		})
	}))
...
func TestRecoverLegacyRunningDoesNotOverwriteNewClaim(t *testing.T) {
	db, err := gorm.Open(sqlite.Open(fmt.Sprintf("file:inventory_legacy_recovery_%s?mode=memory&cache=shared", uuid.New())), &gorm.Config{})
	require.NoError(t, err)
	require.NoError(t, db.AutoMigrate(&InventorySyncTask{}))
	...
	id := uuid.New()
	require.NoError(t, db.Create(&InventorySyncTask{
	...
	var claim sync.Once
	claimApplied := false
	require.NoError(t, db.Callback().Update().Before("gorm:update").Register("inventory_claim_before_legacy_recovery", func(tx *gorm.DB) {
		claim.Do(func() {
			result := tx.Exec(
				"UPDATE inventory_sync_tasks SET locked_by = ?, locked_until = ?, heartbeat_at = ?, execution_id = ?, lock_version = lock_version + 1, updated_at = ? WHERE id = ?",
