...
import { Alert, Badge, Button, Col, Form, Input, InputNumber, Row, Segmented, Space, Switch, Typography } from 'antd';
...
function authStatusBadge(status: ProviderTaobaoTmallAuthStatus | null, checking: boolean, loaded: boolean) {
  ...
  return (
    ...
    >
      <Space direction="vertical" size="middle" style={{ width: '100%' }}>
        <Form.Item
          label="默认采集引擎"
          name="collect_taobao_tmall_default_engine"
          tooltip="采集中心弹窗与采集任务快捷提交淘宝/天猫链接时的默认引擎，提交前仍可临时切换。OpenCLI 使用宿主机已登录浏览器。"
        >
          <Segmented options={collectEngineOptions(engineStatus)} disabled={engineStatusLoading} />
        </Form.Item>
        <Alert
          type={openCliStatus?.ready ? 'success' : 'info'}
          showIcon
          message={
            engineStatusLoading
              ? '正在检测 OpenCLI Bridge'
              : openCliStatus?.ready
                ? 'OpenCLI Bridge 已就绪'
                : openCliStatus?.message || 'OpenCLI Bridge 未启用'
          }
          description={
            engineStatusLoading
              ? '检测完成后会显示主引擎可用状态。'
        ...
        <div className={`tm-collector-auth-panel tm-collector-auth-panel--${authKey}`}>
          <Space direction="vertical" size={4} style={{ width: '100%' }}>
            <Space wrap>
              <Typography.Text strong>Playwright 登录状态</Typography.Text>
        ...
        <Row gutter={16}>
          <Col xs={24} sm={12}>
            <Form.Item label="失败自动重试" name="collect_taobao_tmall_retry_on_failure" valuePropName="checked">
              <Switch defaultChecked />
            </Form.Item>
          </Col>
          <Col xs={24} sm={12}>
        ...
        {batchOpen ? (
          <>
            ...
            <Row gutter={16}>
              ...
              <Col xs={24} sm={12}>
                <Form.Item label="每条间隔最小值（毫秒）" name="collect_taobao_tmall_batch_delay_min_ms">
                  <InputNumber min={0} max={120000} style={{ width: '100%' }} placeholder="3500" />
                </Form.Item>
              </Col>
              <Col xs={24} sm={12}>
                <Form.Item label="每条间隔最大值（毫秒）" name="collect_taobao_tmall_batch_delay_max_ms">
            ...
            <Form.Item label="启用淘宝/天猫批量采集" name="collect_taobao_tmall_batch_enabled" valuePropName="checked">
              <Switch defaultChecked />
            </Form.Item>
            <Form.Item
