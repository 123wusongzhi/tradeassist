...
const recovery4 = process.argv.includes('--recovery4');
...
if (!runtimeMatches && !recovery4) issues.push('runtime source tree fingerprint differs from the frozen baseline');
const report = {
  phase: recovery4 ? 'P7-V2-R3B-FAST-CLOSE' : 'P7-V2-R3B-FIX',
  ...
  recovery4Required: !runtimeMatches,
  ...
  issues,
  nextRequiredAction: issues.length ? 'Restore immutable raw evidence and establish a new baseline when runtime or load semantics differ.' : recovery4 && !runtimeMatches ? 'Recovery4 baseline is required and may begin; Current remains prohibited until that baseline is frozen.' : 'R3B execution may begin manually.',
};
