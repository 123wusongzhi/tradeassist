...
func (s *Service) checkOzonListingConfig(ctx context.Context, p product.Product, requestedShopID *uuid.UUID) []CheckItem {
	...
	if err := s.DB.WithContext(ctx).Where("product_id = ? AND platform = ?", p.ID, "ozon").First(&cfg).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return []CheckItem{{Group: "platform", Code: "OZON_CATEGORY_NOT_SELECTED", Level: levelError, Message: "Ozon 类目未选择", Suggestion: "请先保存商品级 Ozon 类目与属性。"}}
		}
		return nil
	}
	if cfg.ShopID == nil {
		return []CheckItem{{Group: "platform", Code: "OZON_SHOP_NOT_AUTHORIZED", Level: levelError, Message: "Ozon 店铺未选择", Suggestion: "请选择已授权的 Ozon 店铺。"}}
	}
	if requestedShopID != nil && *requestedShopID != uuid.Nil && *cfg.ShopID != *requestedShopID {
		return []CheckItem{{Group: "platform", Code: "OZON_SHOP_CONFIG_MISMATCH", Level: levelError, Message: "商品配置的 Ozon 店铺与本次目标店铺不一致", Suggestion: "请保存当前目标店铺的商品级 Ozon 配置后重试。"}}
	}
	...
	if err := s.DB.WithContext(ctx).Where("platform = ? AND category_id = ?", "ozon", cfg.CategoryID).First(&cat).Error; err != nil {
		return []CheckItem{{Group: "platform", Code: "OZON_CATEGORY_CACHE_MISSING", Level: levelError, Message: "本地没有 Ozon 类目缓存", Suggestion: "请同步 Ozon 类目。"}}
	}
	if !cat.IsLeaf || cat.Status != "active" {
		return []CheckItem{{Group: "platform", Code: "OZON_CATEGORY_INACTIVE", Level: levelError, Message: "Ozon 类目已停用或不是叶子类目", Suggestion: "请重新选择有效 Ozon 类目。"}}
	}
	...
	if len(attrs) == 0 {
		return []CheckItem{{Group: "platform", Code: "OZON_ATTRIBUTE_SCHEMA_MISSING", Level: levelError, Message: "Ozon 类目属性模板尚未同步", Suggestion: "请刷新当前 Ozon 类目的属性模板。"}}
	}
