...
func TestRegisterUnavailableDependenciesFailClosed(t *testing.T) {
	for _, svc := range []*Service{nil, {}} {
		...
		for _, path := range []string{"/api/v1/pricing/calculate", "/api/v1/products/test/pricing/apply"} {
			...
			if err := json.Unmarshal(rec.Body.Bytes(), &body); err != nil || body.Code != response.CodeInternalError || body.Message != "pricing unavailable" {
