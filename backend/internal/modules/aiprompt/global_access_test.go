...
func TestAIPromptRoutesRequireGlobalSettingsAdmin(t *testing.T) {
	gin.SetMode(gin.TestMode)
	db, err := gorm.Open(sqlite.Open("file:aiprompt_global_access?mode=memory&cache=shared"), &gorm.Config{Logger: logger.Default.LogMode(logger.Silent)})
	require.NoError(t, err)
	require.NoError(t, db.AutoMigrate(&admin.AdminUser{}, &AIPrompt{}))
	...
	for _, actor := range []string{"legacy_admin", "tenant_admin"} {
		...
		require.Equalf(t, http.StatusForbidden, rec.Code, "%s must not access instance AI prompts", actor)
	}
	...
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)
	require.Equal(t, http.StatusOK, rec.Code, "system admin must reach instance AI prompts")
}

func TestAIPromptRoutesFailClosedWithoutServiceDependencies(t *testing.T) {
	gin.SetMode(gin.TestMode)
