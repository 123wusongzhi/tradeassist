...
TradeMind 采用 monorepo 组织，核心由 Go backend、React admin、Node Playwright
Collector、PostgreSQL 与 Redis 组成；淘宝/天猫可选使用宿主机 OpenCLI Bridge。
...
Go Gin API
        ├── PostgreSQL + Redis
        ├── Playwright Collector :3001
        └── OpenCLI Bridge :3100（可选，仅淘宝/天猫）
```
...
collector/  Node.js + TypeScript；Playwright Collector、OpenCLI Bridge 与受管适配器
docs/       项目文档
scripts/    本地开发编排脚本
```
...
采集运行时由 backend 按任务已持久化的 `engine` 路由：

- Playwright Collector 是独立 Node.js 服务，使用 Playwright 打开页面并解析商品
  数据，本地和 Docker 都使用 `3001`。
- OpenCLI Bridge 是可选的宿主机轻量进程，当前只支持淘宝/天猫，使用 `3100`，
  不替换 Playwright Collector。
- 两个引擎使用独立客户端和地址；OpenCLI 故障不影响 Playwright，任务运行失败后
  不自动跨引擎回退。
- 不直接操作主业务数据库。
