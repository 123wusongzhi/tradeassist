...
const FIELDS: Record<string, FieldSpec> = {
  ...
  collect_taobao_tmall_default_engine: {},
};
...
const PROVIDER_CARD_DESC: Record<CollectSettingsProviderKey, string> = {
  ...
  taobao_tmall: '双引擎状态、默认引擎与批量采集限速',
...
const AUTH_STATUS_LABEL: Record<
  ...
  {
    text: string;
    badge: 'processing' | 'success' | 'error' | 'warning' | 'default';
  }
> = {
...
function authStatusBadge(status: Provider1688AuthStatus | null, checking: boolean, loaded = true) {
...
function pddAuthStatusBadge(status: ProviderPinduoduoAuthStatus | null, checking: boolean, loaded: boolean) {
  ...
  return (
    ...
    >
      <Space direction="vertical" size="middle" style={{ width: '100%' }}>
        <div className={`tm-collector-auth-panel tm-collector-auth-panel--${authKey}`}>
          <Space direction="vertical" size={4} style={{ width: '100%' }}>
            ...
            {authChecking ? (
              ...
              <Typography.Text type="secondary">请在普通浏览器外，使用采集专用浏览器完成 1688 登录。</Typography.Text>
            )}
        ...
        <Row gutter={16}>
          <Col xs={24} sm={12}>
            <Form.Item label="遇风控 / 验证页自动重试" name="collect_batch_retry_on_blocked" valuePropName="checked">
              <Switch />
            </Form.Item>
          </Col>
          <Col xs={24} sm={12}>
            <Form.Item label="遇超时 / 导航失败自动重试" name="collect_batch_retry_on_timeout" valuePropName="checked">
              <Switch />
            </Form.Item>
          </Col>
        </Row>
    ...
    <ProCard title="自定义链接专属配置" variant="outlined" className="tm-collector-settings__panel">
      <Space direction="vertical" size="middle" style={{ width: '100%' }}>
        ...
        <Typography.Paragraph type="secondary" style={{ marginBottom: 0 }}>
          在「采集规则」页创建并启用规则后，先「测试采集效果」确认标题、价格、图片能识别，再提交采集任务。不会写规则时可使用「AI
          帮我生成规则」。
    ...
    <ProCard title="速卖通专属配置" variant="outlined" className="tm-collector-settings__panel">
      <Space direction="vertical" size="middle" style={{ width: '100%' }}>
        ...
        <Alert
          ...
          showIcon
          message={
            providerRow?.batchSupported ? '速卖通批量采集已开放' : '速卖通批量采集：暂未开放（测试阶段仅支持单条采集）'
          }
        />
        <Form.Item
          label="启用速卖通批量采集（预留）"
      ...
      <Space direction="vertical" size="middle" style={{ width: '100%' }}>
        ...
        <div className={`tm-collector-auth-panel tm-collector-auth-panel--${authKey}`}>
          <Space direction="vertical" size={4} style={{ width: '100%' }}>
            ...
            <Typography.Paragraph type="secondary" style={{ marginBottom: 0 }}>
              拼多多部分商品页和批发页需要登录或微信扫码授权。建议从失败任务或采集弹窗中打开登录，系统会直接打开需要采集的商品页；若无上下文则打开批发入口（不会打开移动端
              App 首页）。系统不会保存账号密码。
            </Typography.Paragraph>
            {authChecking ? (
              <Typography.Text type="secondary">正在检测登录态…</Typography.Text>
              ...
              <Typography.Text type="secondary">请先打开采集浏览器登录拼多多，然后重新检测。</Typography.Text>
        ...
        <Row gutter={16}>
          ...
          <Col xs={24} sm={12}>
            <Form.Item label="风控 / 验证页自动重试" name="collect_pinduoduo_retry_on_blocked" valuePropName="checked">
              <Switch />
            </Form.Item>
          </Col>
        </Row>
        ...
        <Form.Item label="启用拼多多批量采集" name="collect_pinduoduo_batch_enabled" valuePropName="checked">
          <Switch disabled={!providerRow?.batchSupported} />
        </Form.Item>
      </Space>
    </ProCard>
...
export default function CollectorSettingsPage() {
  ...
  const planned = useMemo(() => isPlannedCollectProvider(providers, providerOption), [providers, providerOption]);
  ...
  const loadPddAuthStatus = useCallback(async () => {
    ...
    if (!testUrl) {
      message.warning('未提供商品详情链接，本次只能检测拼多多首页是否可访问，不能准确判断是否已登录。', 6);
    }
  ...
  const loadTbAuthStatus = useCallback(async () => {
    ...
    try {
      const data = await checkTaobaoTmallLogin({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      form.setFieldsValue({
        ...
        collect_batch_concurrency_1688: g.collect_batch_concurrency_1688 ? Number(g.collect_batch_concurrency_1688) : 1,
        ...
        collect_batch_max_retries_1688: g.collect_batch_max_retries_1688 ? Number(g.collect_batch_max_retries_1688) : 2,
        ...
        collect_custom_batch_enabled:
          g.collect_custom_batch_enabled === '1' || g.collect_custom_batch_enabled === 'true',
        ...
        collect_taobao_tmall_access_check_enabled: parseBoolSetting(g.collect_taobao_tmall_access_check_enabled),
        ...
        collect_taobao_tmall_default_engine: normalizeCollectEngine(g.collect_taobao_tmall_default_engine, 'opencli'),
      });
    } catch (e: unknown) {
  ...
  const handleSave = async (values: Record<string, unknown>) => {
    try {
      const payload = {
        ...
        collect_taobao_tmall_access_check_enabled: values.collect_taobao_tmall_access_check_enabled ? '1' : '0',
        ...
        collect_taobao_tmall_detail_image_wait_ms: String(values.collect_taobao_tmall_detail_image_wait_ms ?? 3000),
        ...
        collect_taobao_tmall_batch_delay_min_ms: String(values.collect_taobao_tmall_batch_delay_min_ms ?? 3500),
        collect_taobao_tmall_batch_delay_max_ms: String(values.collect_taobao_tmall_batch_delay_max_ms ?? 6000),
        collect_taobao_tmall_batch_max_retries: String(values.collect_taobao_tmall_batch_max_retries ?? 2),
        collect_taobao_tmall_batch_pause_on_login: values.collect_taobao_tmall_batch_pause_on_login ? '1' : '0',
        collect_taobao_tmall_batch_pause_on_verify: values.collect_taobao_tmall_batch_pause_on_verify ? '1' : '0',
        collect_taobao_tmall_default_engine: normalizeCollectEngine(
          typeof values.collect_taobao_tmall_default_engine === 'string'
            ? values.collect_taobao_tmall_default_engine
            : undefined,
          'opencli',
        ),
      };
            ...
            <Row gutter={[16, 16]} align="stretch">
              <Col xs={24} xl={10}>
                ...
                >
                  ...
                  <Form.Item label="采集服务监听地址" name="collector_http_addr" rules={[{ required: true }]}>
