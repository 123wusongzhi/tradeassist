...
export function evaluateHostIsolationValidationRunnerFinalGate() {
  ...
  const missingIntegrationNeedles = sourceContainsAll(runnerSource, [
    ...
    'waitForPredictiveHostStabilityBarrier',
  ...
  const checks = {
    ...
    runnerVersionTwo: HOST_ISOLATION_VALIDATION_RUNNER_VERSION === 2,
    formalHostIsolationVersionThree: FORMAL_HOST_ISOLATION_VERSION === 3,
    ...
    fixturePredictiveBarrierCovered: fixtureMatrix.predictiveHostStabilityFailureCount === 0,
    fixturePostgresIdentityDistinct: fixtureMatrix.postgresProcessIdentityDistinct === true && fixtureMatrix.postgresDataDirectoryDistinct === true && fixtureMatrix.postgresPortDistinct === true,
