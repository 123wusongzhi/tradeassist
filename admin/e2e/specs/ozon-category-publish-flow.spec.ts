...
test.describe('@ozon-publish Ozon 类目与刊登流程', () => {
  ...
  test('previews per-SKU image order, applies a bulk choice, and saves adjustable selections locally', async ({
    ...
    const red = page.locator(
      'section[aria-label="SKU 图片配置：红色 / M"]',
    );
    const blue = page.locator(
      'section[aria-label="SKU 图片配置：蓝色 / L"]',
    );
    ...
    await page
      .getByRole('button', { name: '保存商品级 Ozon 配置' })
      .click();
    await admin.writeGuard.expectRequestCount('save-ozon-sku-images', 1);
    expect(
      admin.writeGuard.calls('save-ozon-sku-images')[0].postDataJSON
        .ozonImages,
    ).toEqual({
      version: 1,
      skuSelections: [
  ...
  test('can clear stale image selections after the SKU original image changes', async ({
    ...
    await openStage(page, 'config');
    await page
      .getByRole('button', { name: '清除已保存的替代主图' })
      .click();
    await page
      .getByRole('button', { name: '清空该 SKU 的追加图片' })
      .click();
