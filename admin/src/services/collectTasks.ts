...
import type { CollectEngine, CollectEnginesStatus } from '@/utils/collectEngine';

export type { CollectEngine, CollectEngineStatusItem, CollectEnginesStatus } from '@/utils/collectEngine';
...
export async function createCollectTask(body: {
  ...
  engine?: 'playwright' | 'opencli';
}) {
  return postJSON<CollectTaskRow>('/api/v1/collect/tasks', body);
}

export async function fetchCollectEnginesStatus() {
  return getJSON<CollectEnginesStatus>('/api/v1/collect/engines/status');
}
...
export async function queryCollectTaskEvents(taskId: string, params?: { page?: number; pageSize?: number }) {
