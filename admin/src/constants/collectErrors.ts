...
export function mapCollectorErrorCodeLabel(code?: string | null, engine?: string | null): string {
  const c = (code ?? '').trim().toUpperCase();
  const isOpenCli = (engine ?? '').trim().toLowerCase() === 'opencli';
  switch (c) {
    case 'LOGIN_REQUIRED':
      return '页面需要登录';
    ...
    case 'PRODUCT_NOT_FOUND':
      return '商品不存在或已下架';
    case 'ITEM_NOT_FOUND':
      return isOpenCli ? 'OpenCLI 未获取到商品内容' : '商品不存在或已下架';
...
/** 错误码 → 操作建议（主流程展示） */
export function mapCollectorErrorCodeDetail(
  code?: string | null,
  source?: string | null,
  engine?: string | null,
): string {
  ...
  switch (c) {
    case 'LOGIN_REQUIRED':
      return isPdd
        ...
        : isTb
          ? isOpenCli
            ? 'OpenCLI 使用的 Chrome 当前没有可用登录态。请在已连接 OpenCLI 扩展的 Chrome 中登录淘宝/天猫后重试。'
            : '该淘宝/天猫商品页需要登录后才能采集。请打开 Playwright 采集浏览器完成登录后重试。'
    ...
    case 'CAPTCHA':
      return isTb
        ? isOpenCli
          ? 'OpenCLI 使用的 Chrome 出现验证码或安全验证。请在已连接 OpenCLI 扩展的 Chrome 中完成验证后重试。'
          : '淘宝/天猫页面可能出现验证码或安全验证，请在 Playwright 采集浏览器中手动完成验证后重试。'
        : isPdd
          ? '拼多多页面可能触发验证或风控，请稍后重试，或在采集浏览器中手动完成验证。'
          : '目标网站可能出现验证码或安全验证，请稍后重试，或在采集浏览器中手动完成验证。';
    case 'ITEM_NOT_FOUND':
      return isOpenCli
        ? 'OpenCLI 本次没有返回商品内容。这可能由登录态、验证、限流、页面结构变化或商品下架引起，不能仅凭本次结果确认商品已下架。'
        : '商品不存在、已下架或链接无效。';
    case 'MAIN_IMAGES_EMPTY':
      return isTb
    ...
    case 'PARSE_FAILED':
      return isPdd
        ? '部分商品信息未能识别，请采集后人工检查。'
        : isTb && isOpenCli
          ? 'OpenCLI 已打开页面，但没有获得完整商品内容，不能仅凭本次结果确认商品已下架。请确认扩展连接的是你刚才能正常打开商品的那个 Chrome，并检查登录、验证或限流状态；持续出现时可能需要更新适配器。'
    ...
    case 'UNSUPPORTED_PINDUODUO_URL':
      return isPdd
        ...
        : '当前链接类型暂未支持。';
    case 'UNSUPPORTED_TAOBAO_URL':
      return isTb ? '当前链接不是标准淘宝/天猫商品详情页，请复制商品详情页链接后重试。' : '当前链接类型暂未支持。';
    case 'TITLE_NOT_FOUND':
...
export function mapCollectErrorMessage(err: unknown, source?: string | null, engine?: string | null): string {
  const raw = err instanceof Error ? err.message : String(err ?? '');
  const upper = raw.toUpperCase();
  const normalizedSource = (source ?? '').trim().toLowerCase();
  const isTaobaoTmall = normalizedSource === 'taobao_tmall' || normalizedSource === 'taobao';

  if (upper.includes('OPENCLI_BRIDGE_DISABLED')) {
    return mapCollectorErrorCodeDetail('OPENCLI_BRIDGE_DISABLED', source);
  }
  ...
  if (
    upper.includes('OPENCLI_BRIDGE_ERROR') ||
    (isTaobaoTmall &&
      (upper.includes('HOST.DOCKER.INTERNAL:3100') ||
        upper.includes('127.0.0.1:3100') ||
        upper.includes('LOCALHOST:3100')) &&
      (upper.includes('CONNECTION REFUSED') || upper.includes('ECONNREFUSED')))
  ) {
    return mapCollectorErrorCodeDetail('OPENCLI_BRIDGE_ERROR', source);
  }
  if (upper.includes('CONNECTION REFUSED') || upper.includes('ECONNREFUSED') || upper.includes('DIAL TCP')) {
    return '采集服务当前无法连接。请检查对应采集引擎是否已启动，确认状态正常后再重试。';
  }
  ...
  if (upper.includes('LOGIN_REQUIRED')) {
    return mapCollectorErrorCodeDetail('LOGIN_REQUIRED', source, engine);
  }
  if (upper.includes('ITEM_NOT_FOUND')) {
    return mapCollectorErrorCodeDetail('ITEM_NOT_FOUND', source, engine);
  }
  if (upper.includes('EMPTY_RESULT')) {
    return mapCollectorErrorCodeDetail('PARSE_FAILED', source, engine);
  }
  ...
  if (upper.includes('PARSE_FAILED_TITLE_MISSING')) {
    return mapCollectorErrorCodeDetail('PARSE_FAILED_TITLE_MISSING', source, engine);
  }
  ...
  if (upper.includes('PARSE_FAILED_IMAGE_MISSING') || raw.includes('no_main_images')) {
    return mapCollectorErrorCodeDetail('PARSE_FAILED_IMAGE_MISSING', source, engine);
  }
  ...
  if (upper.includes('PAGE_BLOCKED_OR_VERIFY_REQUIRED')) {
    return mapCollectorErrorCodeDetail('PAGE_BLOCKED_OR_VERIFY_REQUIRED', source, engine);
  }
