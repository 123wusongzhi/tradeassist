...
> Current status: **Post-F9 Enhancement** · **MVP Demo Ready** (final acceptance completed on 2026-07-07). **F1–F9 done** · **Tag deferred**. The project is still **not Production Ready** and gray release is not allowed; real Douyin E2E remains blocked by real credentials, so Douyin Shop stays **Release Candidate**. H1.2.1 completed URL state browser spot-check and low-risk fixes; H1.2 added URL state for orders, product drafts, inventory, and customer service list pages; the tag remains deferred while low-risk polish continues on `dev` — [`docs/POST_F9_ENHANCEMENT_PLAN.md`](docs/POST_F9_ENHANCEMENT_PLAN.md). See [`docs/F9_FINAL_ACCEPTANCE_REPORT.md`](docs/F9_FINAL_ACCEPTANCE_REPORT.md), [`docs/H1_2_URL_STATE_BROWSER_CHECK.md`](docs/H1_2_URL_STATE_BROWSER_CHECK.md).
