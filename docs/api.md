...
- 可观测性（P5，需权限）：`GET /api/v1/observability/overview|http|tasks|providers|security`；`GET /api/v1/observability/alerts`；`POST /api/v1/observability/alerts/:id/ack|silence`；内部指标：`GET /internal/metrics`（默认仅内网/本机）
