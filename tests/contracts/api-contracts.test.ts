...
describe("TradeMind API contract registry", () => {
  ...
  it("defines payload/query contracts for state-changing publish APIs", () => {
    ...
    const collectTask = contracts.endpoints.find(
      (item) => routeKey(item) === "POST /api/v1/collect/tasks",
    );
    const collectBatch = contracts.endpoints.find(
      (item) => routeKey(item) === "POST /api/v1/collect/batches",
    );
    const ozonMappings = contracts.endpoints.find(
      (item) =>
        routeKey(item) ===
        "PUT /api/v1/platform/ozon/categories/:id/attribute-mappings",
    );
    const ozonCategories = contracts.endpoints.find(
      (item) => routeKey(item) === "GET /api/v1/platform/ozon/categories",
    );
    const ozonDictionaryValues = contracts.endpoints.find(
      (item) =>
        routeKey(item) ===
        "GET /api/v1/platform/ozon/categories/:id/attributes/:attrId/values",
    );
    ...
    const ozonReadiness = contracts.endpoints.find(
      (item) =>
        routeKey(item) === "POST /api/v1/products/:id/readiness/validate",
    );
    const ozonGroupConfirm = contracts.endpoints.find(
      (item) =>
        routeKey(item) ===
        "POST /api/v1/product-publish/ozon/category-groups/confirm",
    );
    const ozonChanges = contracts.endpoints.find(
      (item) =>
        routeKey(item) === "GET /api/v1/platform/ozon/categories/changes",
    );
    const adminTenantOptions = contracts.endpoints.find(
      (item) => routeKey(item) === "GET /api/v1/admin/tenants",
    );
    const createAdminUser = contracts.endpoints.find(
      (item) => routeKey(item) === "POST /api/v1/admin/users",
    );
    const updateAdminUser = contracts.endpoints.find(
      (item) => routeKey(item) === "PATCH /api/v1/admin/users/:id",
    );
    ...
    expect(ozonCategories?.query).toEqual([
      "keyword",
      "onlyLeaf",
      "activeOnly",
      "limit",
    ...
    expect(publish?.conditionalRequiredHeaders).toEqual({
      ozon: ["Idempotency-Key"],
    });
    ...
    expect(ozonConfig?.responseFields).toEqual(
      expect.arrayContaining([
        ...
        "platformAttributes.skuVariantAttributeIds",
        "platformAttributes.skuAttributeOverrides",
        "ozonListing.skuPriceOverrides",
        "ozonListing.package",
        "ozonPreview.skus[].price",
        "ozonPreview.skus[].localStock",
        "ozonPreview.skus[].stockSource",
        "ozonPreview.skus[].images",
        "ozonPreview.skus[].platformAttributes",
        "ozonPreview.skus[].attributeSources",
        "ozonPreview.canSubmit",
      ]),
    );
    expect(ozonConfig?.errorStatuses).toEqual([400, 403, 404]);
    expect(ozonReadiness?.requestBody).toEqual(["platform", "shopId"]);
    expect(ozonReadiness?.responseFields).toEqual(
      expect.arrayContaining([
        "resolvedOzon.title",
        "resolvedOzon.package",
        "resolvedOzon.skus[].price",
        "resolvedOzon.skus[].localStock",
        "resolvedOzon.skus[].stockSource",
        "resolvedOzon.skus[].images",
        "resolvedOzon.skus[].platformAttributes",
        "resolvedOzon.skus[].attributeSources",
        "resolvedOzon.canSubmit",
      ]),
    );
    expect(ozonReadiness?.errorStatuses).toEqual([400, 403, 404, 502, 503]);
    expect(ozonReadiness?.errorDataFields).toEqual(["errorCode"]);
    expect(ozonGroupConfirm?.requestBody).toEqual([
      "shopId",
      "groups",
      "saveMappings",
    ]);
    expect(ozonChanges?.responseFields).toEqual([
      "list[].categoryName",
      "list[].occurredAt",
      "list[].detail",
    ]);
    expect(adminTenantOptions?.responseFields).toEqual([
      "list[].id",
      "list[].name",
      "list[].shopNames",
    ]);
    expect(createAdminUser?.requestBody).toEqual([
      "email",
      "phone",
      "password",
      "displayName",
    ...
    expect(updateAdminUser?.requestBody).toEqual([
      "displayName",
      "role",
      "status",
      "tenantId",
