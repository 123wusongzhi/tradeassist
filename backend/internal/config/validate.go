...
func (c *Config) Validate() error {
	...
	if err := c.validateCollectEngines(); err != nil {
		return err
	}
...
func (c *Config) validateCollectEngines() error {
	switch strings.ToLower(strings.TrimSpace(c.CollectDefaultEngineTaobaoTmall)) {
	case "", "playwright", "opencli":
	default:
		return fmt.Errorf(
			"%s: COLLECT_DEFAULT_ENGINE_TAOBAO_TMALL must be playwright or opencli",
	...
	if c.OpenCLIBridgeEnabled {
		if strings.TrimSpace(c.OpenCLIBridgeBaseURL) == "" {
			return fmt.Errorf("%s: OPENCLI_BRIDGE_BASE_URL is required when OPENCLI_BRIDGE_ENABLED=true", ErrCodeConfigRequired)
		}
		if err := validateHTTPBaseURL("OPENCLI_BRIDGE_BASE_URL", c.OpenCLIBridgeBaseURL); err != nil {
			return err
		}
	}
	return nil
...
func validateHTTPBaseURL(name, value string) error {
	...
	if err != nil ||
		...
		parsed.User != nil {
		return fmt.Errorf("%s: %s must be an http(s) URL without embedded credentials", ErrCodeConfigInvalid, name)
	}
	return nil
}
