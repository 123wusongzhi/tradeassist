...
function runResolverIfPresent() {
  if (!fs.existsSync(path.join(root, 'scripts/p7-c2-capability-evidence.mjs'))) return null;
  return spawnSync(process.execPath, ['scripts/p7-c2-capability-evidence.mjs'], {
    cwd: root,
    encoding: 'utf8',
...
const resolver = runResolverIfPresent();
...
const checks = [
  ...
  check('race-mapping-11', race.mapped === 11, `mapped=${race.mapped}`),
  check('linux-race-passed', race.status === 'passed' && race.environmentBlocked === false && race.executed === 11 && race.dataRaces === 0 && race.deadlocks === 0, `race status=${race.status}`),
];
...
const closureReport = {
  ...
  c2EvidenceResolverExitCode: resolver?.status ?? null,
...
const audit = {
  ...
  summary: {
    total: 57,
    mandatoryImplemented: norm.capabilities?.mandatoryImplemented ?? 0,
