...
- `collector/`：Node.js + TypeScript + Playwright 商品采集服务，运行时代码位于 `collector/src/**`。
...
Collector 当前围绕 `providers/**`、`browser/**`、`normalizer/**`、`tasks/**`、`types/**`、`config/**` 组织。新增来源 adapter 时应在 adapter 边界解析第三方结构，并尽早归一化，不把原始平台结构扩散到全部模块。
