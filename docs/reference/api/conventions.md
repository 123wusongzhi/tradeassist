...
刊登提交 `POST /api/v1/products/:id/publish` 会保存 `product_publish_tasks`。`status` 是队列/执行态；运营页面必须以 `publishStatus` 为业务事实态，其值包括 `draft` / `checking` / `ready` / `publishing` / `imported` / `pending_review` / `needs_action` / `result_unknown` / `published` / `failed` / `cancelled`。任务 DTO 还包含 `productId`、`targetPlatform`、`targetStoreId`、`publishMode`、`title`、`description`、`images`、`skus`、`price`、`currency`、`checkResult`、`platformPayload`、`platformResult`、`platformProductId`、`retryable`、`recoveryState`、`errorCode`、`errorMessage`、`createdAt`、`updatedAt`。平台字段映射快照包含 `platformTitle`、`platformDescription`、`platformImages`、`platformSkus`、`platformPrice`、`platformStock`、`platformCategory`、`platformAttributes`。

对于 Ozon，创建记录只表示“已创建提交，等待处理”。取得真实 `platformProductId` 且 import 完成只可显示 `imported`（已创建、尚未验证可售），警告或库存同步异常为 `needs_action`；只有只读回查或人工证据明确 `sellableVerified=true` 时才可显示 `published`。mutation 后超时、租约丢失或结果无法确认统一为 `result_unknown`，禁止自动重试；只有明确未发送 mutation 或人工确认平台未创建后，服务端才可保存 `retryable=true`。运行中的 Ozon 任务不可取消，避免本地假取消与平台实际创建并存。
...
| `PUT` | `/api/v1/shops/:id/auth` | 保存店铺凭证。Ozon 的 `appKey` 为 Client ID、`accessToken` 为 Api-Key；保存只证明密文已持久化，店铺会保持/回到 `authStatus=unauthorized`，不会因此宣称授权成功。更新时空白或脱敏敏感字段沿用既有密文，首次配置仍必须提供必填凭证。 |
| `POST` | `/api/v1/shops/:id/test-connection` | 使用已保存凭证做只读连接测试。只有 Provider 明确返回 `ok=true` 才写入 `authStatus=authorized` 并同步可用的店铺名、平台店铺编号、地区和币种；Provider 返回 `ok=false` 时写入 `authStatus=invalid`、记录失败审计并返回 HTTP 400 统一 envelope，不得显示连接成功。 |
...
| `GET` | `/api/v1/platform/ozon/categories` | 读取本地缓存的 Ozon 类目树；同步时按 Ozon 官方类目树的递归 `children` 结构解析，并使用简体中文语言参数。支持 `keyword`、`parentId`、`rootOnly`、`onlyLeaf`、`activeOnly`、`limit`、`offset`：`rootOnly=1` 只返回一级节点，`parentId` 只返回该父节点的直接子节点，供运营从父类目开始动态 N 级选择；`keyword` 会匹配叶子名称、完整路径中的父级名称与类目 ID。叶子类目 ID 为 `<description_category_id>:<type_id>` 复合键；列表项含 `descriptionCategoryId` / `typeId`、稳定 `path`、`hasChildren`、`childCount`，以及从根节点到当前节点父级的 `ancestors[] { categoryId, name, level }`，搜索结果只能据此还原路径，不能绕过人工叶子确认。分页响应另含缓存总量 `total`、全局叶子数 `leafCount`、本次匹配数 `matchedTotal`、`offset`、`limit`、`lastSyncedAt`、`cacheStale`。平台缺少中文时保留原文，不由客户端臆造翻译。失效类目在缓存中的 `status` 为 `inactive`，对应同步差异事件为 `deactivated`，保存商品配置时由服务端拒绝选用。 |
| `GET` | `/api/v1/platform/ozon/warehouses` | 按必填 `shopId` 实时只读加载当前租户、当前店铺可用的 Ozon 仓库；要求店铺操作权限。响应 `{ list: [{ id, name }] }`，不返回凭证；失败、空列表或已保存 ID 不在列表时，刊登页必须 fail-closed，禁止改为自由输入绕过。 |
...
| `GET` | `/api/v1/platform/ozon/categories/:id/attributes` | 读取某个 Ozon 叶子类目的属性模板缓存（`platform_category_attributes`）；`list[]` 除 `required`、`dictionaryId`、`options` 外，稳定返回 `description`、`valueType`、`skuVariantEligible`、`skuVariantEligibilityKnown`、`isCollection`、`maxValueCount`、`attributeComplexId`、`complexIsCollection`、`categoryDependent`。只有 `skuVariantEligibilityKnown=true && skuVariantEligible=true` 的普通属性可作 SKU 维度；缺失资格默认禁止。响应另含 `variantPolicy`：`maxSkuCount`、`maxVariantAttributeCount`、`maxVariantCombinationCount`、`eligibleAttributeCount`、`variantEligibilityFullyKnown`、`source=ozon_is_aspect+trademind_import_guardrail`。其中资格来自 Ozon `is_aspect`，100 条 SKU/组合阈值是 TradeMind 单次导入保护，不得伪装成平台类目字段。`cacheStale` 提示超过 24h。 |
...
| `GET` | `/api/v1/platform/ozon/category-mappings` | 读取当前租户的本地来源类目 → Ozon 叶子类目映射；可按 `shopId` 筛选。映射是租户拥有的数据，店铺级映射优先于租户默认映射。证据 DTO 另含 `descriptionCategoryId`、`typeId`、`scope`、`selectionMethod`、`confirmationReason`、`templateSyncedAt`、`confirmedAt`、`confirmedBy` 和 `schemaHash`。 |
...
| `PUT` | `/api/v1/platform/ozon/category-mappings` | 保存人工确认的映射；body 含 `shopId?`、`sourceCategoryKey`、`sourceCategoryName?`、`categoryId`、`categoryPath?`、`status?`、`selectionMethod?`（`manual` / `recommended_then_manual`）和不超过 1000 字的 `confirmationReason?`。只能指向活动 Ozon 叶子类目；完整路径、`schemaHash`、模板同步时间、确认人和确认时间均由服务端生成或校准。历史记录可无理由读取，但发布门禁会 fail-closed，要求运营重新逐级核对并补齐 `confirmationReason`。 |
| `GET` | `/api/v1/products/:id/platform-configs/:platform` | 读取商品的平台刊登准备配置。Ozon 调用应传查询参数 `shopId`，按商品 + 平台 + 店铺精确读取；响应含 `legacyFallback` 说明是否临时读取旧商品级配置。除稳定 `id`、类目/schema 外，返回版本化 `platformAttributes` v3：商品级普通多值 `attributes`、可重复 `complexGroups`、SKU 变体维度 `skuVariantAttributeIds[]`，以及按本地 SKU UUID 保存的 `skuAttributeOverrides`。旧单值和 v2 配置继续读取并规范化为 v3；旧多 SKU 配置没有明确逐 SKU 变体值时允许进入编辑，但禁止提交。`ozonPreview.skus[]` 还返回各 SKU 的最终 `platformAttributes`、`attributeSources`、价格/库存/图片及错误；商品级标题/描述/币种/重量尺寸/仓库/VAT 同样带值来源。旧商品没有新版图片选择时只使用各 SKU 原图，不默认追加公共图。 |
| `PUT` | `/api/v1/products/:id/platform-configs/:platform` | 保存商品的平台刊登准备配置。Ozon body 必须含 `shopId` 与活动叶子类目，可保存 `platformAttributes: { version: 3, attributes: { attrId: [{ value, dictionaryValueId? }] }, complexGroups: [{ complexId, attributes }], skuVariantAttributeIds: [attrId], skuAttributeOverrides: { skuUuid: { attrId: [{ value, dictionaryValueId? }] } } }`、版本化 `ozonImages`，以及 `ozonListing: { version: 1, titleOverride?, descriptionOverride?, currencyCode?, skuPriceOverrides, package: { weightG?, widthMm?, heightMm?, depthMm?, warehouseId?, vat? } }`。SKU 变体维度只能选择当前类目实时模板中 `is_aspect=true` 的普通属性；资格为 false 或缺失时均拒绝。保存会拒绝未知商品 SKU、未知属性、未知 `valueType` 和已填写但类型/格式不合法的值，但仍允许暂存缺少必填值的草稿；完整性在预检阶段硬阻断。SKU 售价覆盖只作用于当前 Ozon 店铺；库存字段不在配置中，仍以 `product_skus.stock` 和既有调整审计链路为唯一来源。SKU/图片必须属于当前商品；原始主图优先，只有缺原图时才允许显式替代，公共图按保存顺序追加并做 URL 去重。`product_platform_publish_configs` 的唯一键为 `(product_id, platform, config_scope_key)`，Ozon 的 scope 为店铺 UUID，旧行保留安全回退并在下次保存时物化为店铺级行，不会让多个 Ozon 店铺互相覆盖。保存只写 TradeMind，不创建刊登提交、不调用 Ozon 商品或库存写接口。 |
| `POST` | `/api/v1/products/:id/readiness/validate` | 对指定 `platform=ozon`、`shopId` 做实时只读发布前检查；响应除 `checks[]`、`schemaHash`、`schemaChanged` 外返回 `resolvedOzon`，逐 SKU 给出最终有效价格、库存、图片、平台属性、值来源与错误，并给出标题/描述/币种/包裹配置的最终值和来源。重量尺寸/仓库先取商品 + Ozon 店铺配置，再取全局 `platform_publish_ozon` 预设；仍缺失即阻断。多 SKU 要求至少一个实时 `is_aspect=true` 的普通属性变体维度、每个 SKU 都有值且最终组合唯一，并执行单次 100 SKU/组合的 Adapter 安全阈值；复杂属性、品牌/产地等非 aspect 属性或缺少资格证据的属性不能被静默当成 SKU 维度。多值、复杂组合、字典值、`valueType`、必填属性、类目/schema 与图片计划均按实时模板校验。预览、此预检和任务不可变快照共用 `ResolveOzonListing`；Adapter 只消费每个 SKU 的已解析属性快照，并在 `/v3/product/import` 前再次拉取实时 schema，重复校验 schema hash、`is_aspect`、类型和数量边界。预检只调用 seller info、类目/属性/字典等 Ozon 只读接口，不调用商品导入或库存接口；权限继续执行既有商品操作保护，并校验所选店铺操作权限。 |
...
| `POST` | `/api/v1/product-publish/tasks/:id/cancel` | 取消允许取消的 pending/running 刊登任务；运行中的 Ozon 任务拒绝取消，防止平台 mutation 已发送后出现假取消。 |
...
| `GET` | `/api/v1/product-publish/tasks/:id` | 任务详情（含 `platformPayload`、`platformProductId`、业务事实态 `publishStatus`、`retryable` 与 `recoveryState`；管理端不得只用队列态 `status=success` 推断 Ozon 已可售） |
| `POST` | `/api/v1/product-publish/tasks/:id/retry` | 重试 failed 任务 |
| `POST` | `/api/v1/product-publish/tasks/:id/reconcile-ozon` | 仅对当前租户内 failed Ozon 任务记录人工核对事实，不调用 Ozon、不自动重试。body：`outcome=platform_created|platform_not_created`、必填 `evidence`（最多 1000 字符），可选 `externalProductId` / `externalSpuId` / `externalUrl`、`platformStatus=imported|pending_review|needs_action|sellable`、`sellableVerified`。确认平台已创建必须有平台商品 ID；确认 `sellable` 必须 `sellableVerified=true`。`platform_not_created` 只把任务标为可重试。需要当前租户、店铺 operate 与商品 operate 权限，并写操作审计。 |
...
| `GET` | `/api/v1/platform/douyin/release-gate` | 发布门禁清单：`overallConclusion`、`items[]`（`key` / `label` / `status` / `message`）；`credentials` 项在无真实 E2E 时为 `blocked` |
...
## Backup / Restore / Release / DR API

All write operations require Bearer authentication and backend RBAC. The frontend never receives shell commands, full backup paths, storage secrets or database credentials.
...
Backup / restore / release / DR APIs exist for controlled ops workflows. Historical drill evidence may live under `docs/archive/` or `artifacts/`; real production restore, PITR drill, or traffic switch is out of scope for everyday use and must be planned deliberately.

## Performance / Capacity API Status

Backend configuration, local rate-limit middleware, and guarded load scripts may exist, but public performance management APIs are limited or planned. Do not treat harness evidence as production capacity certification.
