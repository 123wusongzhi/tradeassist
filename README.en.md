...
> Current status: **Post-F9 Enhancement** · **MVP Demo Ready** (final acceptance completed on 2026-07-07). **F1–F9 done** · **Tag deferred**. The project is still **not Production Ready** and gray release is not allowed; real Douyin E2E remains blocked by real credentials, so Douyin Shop stays **Release Candidate**. **H1.4** order/exception URL state and keyword UX safety; H1.3 AI image warning convergence; H1.2.1 URL state browser spot-check; tag remains deferred — [`docs/POST_F9_ENHANCEMENT_PLAN.md`](docs/POST_F9_ENHANCEMENT_PLAN.md), [`docs/H1_4_URL_KEYWORD_RESPONSIVE_CHECK.md`](docs/H1_4_URL_KEYWORD_RESPONSIVE_CHECK.md).
