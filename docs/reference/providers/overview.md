...
- 复杂、多值与 SKU 变体属性：缓存/API 保留 Ozon 的 `is_collection`、`max_value_count`、`attribute_complex_id`、`complex_is_collection` 和 `category_dependent`。商品配置使用 `platformAttributes.version=3`：商品级普通属性保存值数组，复杂属性保存显式、可重复的 `complexGroups`，多 SKU 另以 `skuVariantAttributeIds` 指定普通属性维度，并在 `skuAttributeOverrides[skuUuid]` 保存每个 SKU 的 Ozon 值。Admin 可从本地 SKU `attrs` 做严格同名/词典值自动匹配，未匹配项必须人工选择；`category_dependent` 只保留其类目依赖语义，不会被推断成变体。多 SKU 缺少维度、任一 SKU 缺值或最终组合重复都会阻断。`attribute_complex_id > 0` 的组合属性仍是商品级可重复字段组，不能作为 SKU 维度；若类目必填语义无法正确表达，明确指出属性并停止提交。非 collection 多值、超过最大值数、不可重复 complex group、组合内必填缺失或字典 ID 不匹配同样阻断，绝不按单值静默提交。旧单值与 v2 配置继续读取并规范化为 v3。
...
- 状态边界：“保存当前编辑（不提交）”只保存 TradeMind 的商品 + Ozon 店铺配置，不创建处理记录、不调用 Ozon 写接口。旧本地能力若保留，只在高级操作显示为“保存本地快照（不会提交 Ozon）”。“检查并进入提交确认”调用只读后端预检；真实提交必须二次确认，之后才创建 TradeMind 刊登提交并由 Worker 调用 Ozon。仅当 Ozon 返回真实 product ID 后才可声明“Ozon 商品已创建，等待平台审核”。
- 重试与人工核对：Ozon 真实提交强制要求 `Idempotency-Key`。Ozon 返回 `failed` 或结果未知时系统不自动重试；刊登进度仅在服务端明确标记 `retryable=true` 时显示手动重试，否则显示人工核对说明，也不能用新 `Idempotency-Key` 绕过。运营者必须按 `offer_id` / 平台商品 ID 在 Ozon 与刊登进度中核对真实结果。
