...
| Ozon Provider / 类目刊登 | `backend/internal/providers/platform/ozon`、`backend/internal/modules/shop/ozon_category.go` / `ozon_mapping.go` / `ozon_sync_worker.go`、`backend/internal/modules/product/platform_config.go` / `ozon_images.go`、`backend/internal/modules/productcheck/ozon_*`、`backend/internal/modules/productpublish/draft_builder.go` / `ozon_category_groups.go`、`platform_publish_ozon` 设置、`admin/src/services/ozonCategories.ts` / `ozonPublish.ts`、`admin/src/pages/Settings/Platforms/index.tsx` / `OzonCategoryPanel.tsx`、`admin/src/pages/Settings/Integrations/index.tsx`、`admin/src/pages/Shops/index.tsx`、`admin/src/utils/platformSettings.ts`、`admin/src/pages/Product/OzonPublish`、类目/属性/映射/运行缓存表、`docs/provider.md`、`docs/OZON_CATEGORY_PUBLISH_FLOW_ACCEPTANCE.md`；平台接入页只说明店铺级授权并引导，不保存 Ozon 凭证；修改真实提交时同时检查逐 SKU 图片顺序/缺图阻断、幂等键、不可变快照、schema 漂移、Worker 重试和外部结果落库顺序 |
