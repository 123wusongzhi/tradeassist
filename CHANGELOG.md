...
- Split the collection runtime contract into the always-on Playwright Collector
  (`3001`) and the optional host-side OpenCLI Bridge (`3100`).
- Route and persist the engine per task; OpenCLI currently supports
  `taobao_tmall` only and never silently falls back to Playwright after execution starts.
- Keep Docker's Playwright path on `collector:3001` while optional OpenCLI tasks use
  `host.docker.internal:3100`, so a stopped host Bridge does not affect core collection.
- Make OpenCLI status checks passive with `opencli daemon status`; ordinary Admin
  navigation no longer runs `opencli doctor` or leases a Chrome window.
- Added `docs/collector-engines.md` and synchronized local/Docker startup, environment,
  architecture, Provider, API, migration, troubleshooting, and human acceptance guidance.
