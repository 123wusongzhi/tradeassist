...
import { Alert, Button, Col, Form, Input, Row, Segmented, Select, Space, Switch, Tag, Typography, message } from 'antd';
...
import { normalizeSource, parsePositiveInt, resolveCollectPlatformFromQuery } from '@/utils/urlState';
...
import {
  createCollectTask,
  fetchCollectEnginesStatus,
  fetchCollectTasks,
  retryCollectTask,
  type CollectEnginesStatus,
  type CollectTaskRow,
...
import { formatRuleDomainMismatchMessage, ruleMatchesURL, suggestRuleDomainForHost } from '@/utils/collectRuleMatch';
...
export default function CollectTasksPage() {
  ...
  } = useUrlQueryState<Record<(typeof COLLECT_TASK_QUERY_KEYS)[number], string | undefined>>(COLLECT_TASK_QUERY_KEYS);
  const navSource = normalizeSource(urlState.source);
  const batchIdFromQuery = urlState.batchId;
  const collectPlatformFromQuery = resolveCollectPlatformFromQuery(urlState.sourcePlatform, urlState.source);
  ...
  const formEngine = Form.useWatch('engine', form);
  const [pddUseBrowserProfile, setPddUseBrowserProfile] = useState(false);
  const isPddSource = formSource === 'pinduoduo' || formSource === 'pdd';
  const isTaobaoTmallSource = formSource === OPENCLI_SUPPORTED_SOURCE;
  const [defaultEngine, setDefaultEngine] = useState<CollectEngine>('opencli');
  ...
  // 淘宝/天猫支持双引擎：选中该采集服务时用「采集设置」里的默认引擎填充，
  // 用户提交前仍可手动切换；切走其他平台时清掉引擎，避免误带。
  useEffect(() => {
    if (isTaobaoTmallSource) {
      if (!engineResolved) return;
      if (!form.getFieldValue('engine')) {
        form.setFieldValue('engine', defaultEngine);
      }
    } else if (form.getFieldValue('engine')) {
      form.setFieldValue('engine', undefined);
    }
  }, [isTaobaoTmallSource, defaultEngine, engineResolved, form]);
  ...
  const columns: ProColumns<CollectTaskRow>[] = [
    ...
    {
      title: '执行引擎',
      dataIndex: 'requestOptions',
      width: 116,
      search: false,
      responsive: ['md'],
      render: (_, row) => {
        if (row.source !== OPENCLI_SUPPORTED_SOURCE) return '—';
        const taskEngine = readCollectTaskEngine(row.requestOptions);
        return taskEngine ? (
          <Tag color={taskEngine === 'opencli' ? 'geekblue' : undefined}>{collectEngineLabel(taskEngine)}</Tag>
        ) : (
          <Typography.Text type="secondary">历史未记录</Typography.Text>
        );
      },
    },
    {
      ...
      render: (_, row) => {
        ...
        return (
          ...
          >
            ...
            <Typography.Text copyable={{ text: url, tooltips: false }} style={{ flexShrink: 0, margin: 0 }} />
          </div>
        );
      },
    },
    {
      ...
      valueType: 'select',
      valueEnum: {
        ...Object.fromEntries(Object.entries(COLLECT_TASK_STATUS).map(([k, v]) => [k, { text: v.text }])),
    ...
    },
    {
      ...
      render: (_, row) => {
        ...
        const taskEngine = readCollectTaskEngine(row.requestOptions);
        return mapCollectorErrorCodeLabel(row.collectorErrorCode, taskEngine) || row.failureHint || '—';
      },
    },
    {
      title: '处理建议',
      ...
      render: (_, row) => {
        ...
        const hint =
          row.failureHint ||
          mapCollectorErrorCodeDetail(row.collectorErrorCode, row.source, taskEngine) ||
          (row.errorMessage ? mapCollectErrorMessage(row.errorMessage, row.source, taskEngine) : '');
        return hint || '—';
      },
    },
    {
        ...
        >
          <Row gutter={[16, 0]} align="bottom">
            <Col xs={24} sm={12} md={8} lg={6}>
              <Form.Item label="采集平台" name="source" rules={[{ required: true, message: '请选择采集平台' }]}>
            ...
            {isTaobaoTmallSource ? (
              <Col xs={24} sm={12} md={8} lg={6}>
                <Form.Item
                  label="采集引擎"
                  name="engine"
                  tooltip="opencli 使用本机已登录浏览器采集；默认值可在「设置 → 采集设置」调整。"
                >
                  <Segmented<CollectEngine> disabled={!engineResolved} options={collectEngineOptions(engineStatus)} />
                </Form.Item>
              </Col>
            ) : null}
          ...
          {isTaobaoTmallSource && formEngine === 'opencli' ? (
            <Alert
          ...
          {isPddSource && pddUrlHint ? (
            <Alert
              type={pddUrlType === 'goods_detail' ? 'success' : pddUrlType === 'wholesale_detail' ? 'warning' : 'info'}
              showIcon
              message={pddUrlHint}
      ...
      <ProTable<CollectTaskRow>
        ...
        request={async (params) => {
          const qp = {
            ...
            source: (params.source as string | undefined)?.trim() || collectPlatformFromQuery,
