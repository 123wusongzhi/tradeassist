...
export function isSupported1688URL(raw: string): boolean {
  try {
    ...
    return /\/offer\//i.test(path) || /offerId=/i.test(search) || /offer(?:id)?\.html$/i.test(path);
  } catch {
    return false;
  }
}
...
export async function collect1688Page(_options?: { maxPriceProbes?: number }): Promise<NormalizedProduct> {
  ...
  const skuPriceFromBucket = (bucket: Record<string, unknown>): number | undefined =>
    parsePrice(bucket.price) ??
    parsePrice(bucket.discountPrice) ??
    parsePrice(bucket.salePrice) ??
    parsePrice(bucket.priceDisplay) ??
    parsePrice(bucket.consignPrice) ??
    parsePrice((bucket.priceMoney as Record<string, unknown> | undefined)?.value);
