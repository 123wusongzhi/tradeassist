...
func (s *Service) GetFailureDetail(c *gin.Context, taskTypeRaw string, id uuid.UUID) (*FailureDetailDTO, error) {
	...
	if err := s.ClassifyOne(c.Request.Context(), tenantID, &base); err != nil {
		return nil, err
	}
...
func (s *Service) upsertFailureMark(ctx context.Context, tenantID int64, taskType, sourceID, sourceTable, markType, remark string, admin *uuid.UUID) error {
	...
	return s.DB.WithContext(ctx).Model(&TaskFailureMark{}).
		Where("id = ? AND tenant_id = ?", cur.ID, tenantID).
...
func (s *Service) batchApplyMark(c *gin.Context, req BatchMarkRequest, markType, auditAction string) BatchRetryResponse {
	...
	admin := adminFromGin(c)
	ctx := c.Request.Context()
	tenantID, tenantErr := tenantIDFromGin(c)
	if tenantErr != nil {
		for _, it := range req.Items {
			out.Results = append(out.Results, BatchRetryOneResult{TaskType: it.TaskType, ID: it.ID, OK: false, Error: tenantErr.Error()})
			out.FailedCount++
		}
		return out
	}
