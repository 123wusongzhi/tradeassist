...
func TestWriteRoutesFailClosedWithoutDependencies(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			router.ServeHTTP(rec, httptest.NewRequest(http.MethodPost, "/api/v1/shops/test/sync-customer-messages", nil))

			require.Equal(t, http.StatusInternalServerError, rec.Code)
			var envelope response.Envelope
			require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &envelope))
			require.Equal(t, response.CodeInternalError, envelope.Code)
			require.Equal(t, "customer message sync unavailable", envelope.Message)
		})
	}
}

func TestTaskReadRoutesRequireCustomerView(t *testing.T) {
	gin.SetMode(gin.TestMode)
	db, err := gorm.Open(sqlite.Open("file:customersync_read_guard?mode=memory&cache=shared"), &gorm.Config{})
	...
	for _, path := range []string{"/api/v1/customer/message-sync/tasks", "/api/v1/customer/message-sync/tasks/" + uuid.NewString()} {
		rec := httptest.NewRecorder()
