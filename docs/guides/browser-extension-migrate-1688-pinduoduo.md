...
const isHost = host === '1688.com' || host.endsWith('.1688.com');
...
- Chrome MV3 对 host 匹配有规则；`*.1688.com` 是否覆盖 `detail.1688.com` 以实测为准，必要时**显式列出** `detail.1688.com`、`m.1688.com` 等。
...
提交结果时 `product.source` 必须与 task.source **一致**（Backend 会以 task 为准校验/normalize）。
