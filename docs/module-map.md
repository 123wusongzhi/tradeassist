...
| Ozon Provider / 统一刊登中心 | `backend/internal/providers/platform/ozon`、`backend/internal/modules/shop/ozon_category.go` / `ozon_mapping.go` / `ozon_sync_worker.go`、`backend/internal/modules/product/platform_config.go` / `ozon_images.go` / `ozon_listing.go` / `ozon_attributes.go`、`backend/internal/database/migrate_product_platform_config_scope.go`、`backend/internal/modules/productcheck/ozon_*`、`backend/internal/modules/productpublish/draft_builder.go` / `ozon_category_groups.go` / `service_create.go` / `service_worker.go`、`platform_publish_ozon` 设置、`admin/src/services/ozonCategories.ts` / `ozonPublish.ts`、`admin/src/pages/Product/PublishingCenter`、`admin/src/pages/Product/PublishTasks`、商品草稿列表/详情入口、`admin/src/pages/Settings/Platforms/index.tsx` / `OzonCategoryPanel.tsx`、`admin/src/pages/Shops/index.tsx`、`admin/src/utils/platformSettings.ts`、类目/属性/映射/运行缓存表、`product_platform_publish_configs.config_scope_key` / `listing_config`、`docs/api.md`、`docs/provider.md`、`docs/OZON_CATEGORY_PUBLISH_FLOW_ACCEPTANCE.md`；修改时同时检查店铺级唯一键/旧行回退、SKU 售价覆盖与本地库存唯一来源、图片稳定顺序/缺图阻断、属性 collection/complex 元数据、统一解析器、只读预检、不可变快照、Adapter、幂等键、`retryable` 与外部结果落库顺序。平台接入页只说明店铺级授权并引导，不保存 Ozon 凭证；其他平台不得用占位字段伪装为完整刊登能力。 |
...
| Collector Provider / Engine | `collector/`、`collector/src/opencli-bridge/`、`collector/opencli-adapters/`、`backend/internal/modules/collect/`、`backend/internal/modules/collectbrowserprofile/`、采集任务 API、队列、`.env*.example`、`docker-compose.full.yml`、raw 原始数据、`docs/custom-collect-rules.md`、`docs/collector-engines.md`、`docs/provider.md`、`docs/api.md`、`docs/development.md`、`docs/docker-deployment.md`；涉及登录 Profile 时检查 `collect_profile.manage`、Profile key 的租户隔离及系统域 `tenant_id=0`，**1688 改解析时必读 [`docs/collector-1688-pitfalls.md`](collector-1688-pitfalls.md)** |
...
| 安全 / 租户 / 密钥 / Token | `backend/internal/middleware/jwt.go`、`backend/internal/pkg/security`、`backend/internal/pkg/adminperm`、认证会话与注册、OAuth state / 店铺 Token、任务 Worker 租户上下文、加密、脱敏、日志、环境模板、`SECURITY.md`、`docs/api.md`、相关 settings 文档；改动租户字段时同时检查迁移在 SQLite 与 PostgreSQL 的实际执行 |
