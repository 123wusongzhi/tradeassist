...
func Register(r gin.IRouter, dep *Deps) (*collect.Service, *imagetask.Service, *ordersync.Service, *customersync.Service, *productpublish.Service, *inventory.Service, *taskcenter.Service, *douyinruntime.Service, *webhook.Service, *files.Service, *securitymod.Service, *shop.Service) {
	if dep == nil {
		dep = &Deps{}
	}
	platformp.Bootstrap()
	...
	opencliBridgeToken := ""
	opencliBridgeTimeout := 120 * time.Second
	defaultTaobaoTmallEngine := collect.CollectEngineOpenCLI
	if dep.Config != nil {
		opencliBridgeEnabled = dep.Config.OpenCLIBridgeEnabled
		if dep.Config.OpenCLIBridgeBaseURL != "" {
			opencliBridgeBase = dep.Config.OpenCLIBridgeBaseURL
		}
		opencliBridgeToken = dep.Config.OpenCLIBridgeToken
		if dep.Config.OpenCLIBridgeTimeoutSeconds > 0 {
			opencliBridgeTimeout = time.Duration(dep.Config.OpenCLIBridgeTimeoutSeconds) * time.Second
		}
		if dep.Config.CollectDefaultEngineTaobaoTmall != "" {
			defaultTaobaoTmallEngine = dep.Config.CollectDefaultEngineTaobaoTmall
		}
	}
	opencliBridgeClient := collect.NewOpenCLIBridgeClient(
		opencliBridgeBase,
		opencliBridgeToken,
		opencliBridgeTimeout,
	)
	...
	platformozon.BindAIGateway(aiGateway)
	...
	return collectSvc, imageTaskSvc, orderSyncSvc, customerSyncSvc, productPublishSvc, inventorySvc, tcSvc, douyinRuntimeSvc, webhookSvc, fileSvc, secSvc, shopSvc
}
